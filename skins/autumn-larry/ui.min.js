...
*/
function rcube_mail_ui(){function e(a){p||(p=rcmail.local_storage_get_item("prefs.autumn-larry",{}));if(null==p[a]){var c=rcmail.get_cookie(a);null!=c&&(p[a]=c,rcmail.local_storage_set_item("prefs.autumn-larry",p)&&rcmail.set_cookie(a,c,new Date))}return p[a]}function q(a,c){p[a]=c;if(!rcmail.local_storage_set_item("prefs.autumn-larry",p)){var d=new Date;d.setYear(d.getFullYear()+1);rcmail.set_cookie(a,c,d)}}function r(a){a=a?10:0;rcmail.resize_timeout&&window.clearTimeout(rcmail.resize_timeout);rcmail.resize_timeout=
...
F(a.item)}).addEventListener("aftertoggle-editor",function(a){window.setTimeout(function(){t()},200);a&&a.mode&&$("select[name='editorSelector']").val(a.mode)}).addEventListener("compose-encrypted",function(a){$("select[name='editorSelector']").prop("disabled",a.active);$("a.button.attach, a.button.responses")[a.active?"addClass":"removeClass"]("disabled");$("#responseslist a.insertresponse")[a.active?"removeClass":"addClass"]("active")}),H(),$("#composeoptionstoggle").click(function(a){var b=$("#composeoptions").toggle().is(":visible");
$("#composeoptionstoggle").toggleClass("remove").attr("aria-expanded",b?"true":"false");t();q("composeoptions",b?"1":"0");rcube_event.is_keyboard(a)||this.blur();return!1}).css("cursor","pointer"),"0"!==e("composeoptions")&&$("#composeoptionstoggle").click(),(a=rcmail.opener())&&"compose"==a.env.action&&$("#composeoptionstoggle",opener.document).hasClass("remove")&&$("#composeoptionstoggle").click(),(new rcube_splitter({id:"composesplitterv",p1:"#composeview-left",p2:"#composeview-right",orientation:"v",
relative:!0,start:206,min:170,size:12,render:t})).init(),$("#attachment-list > li").each(function(){F(this)})):"bounce"==rcmail.env.action?H():"list"!=rcmail.env.action&&rcmail.env.action?"get"==rcmail.env.action&&(new rcube_splitter({id:"mailpartsplitterv",p1:"#messagepartheader",p2:"#messagepartcontainer",orientation:"v",relative:!0,start:226,min:150,size:12})).init():(w(),$("#maillistmode").addClass(rcmail.env.threading?"":"selected").click(function(a){A("list");return!1}),$("#mailthreadmode").addClass(rcmail.env.threading?
"selected":"").click(function(a){A("thread");return!1}),rcmail.init_pagejumper("#pagejumper"),rcmail.addEventListener("setquota",D).addEventListener("layout-change",w));$("#mailview-left").length&&(new rcube_splitter({id:"mailviewsplitterv",p1:"#mailview-left",p2:"#mailview-right",orientation:"v",relative:!0,start:206,min:150,size:12,callback:f,render:n})).init()}else if("settings"==rcmail.env.task)if(rcmail.addEventListener("init",function(){var a="#settingstabpreferences";rcmail.env.action&&(a=
"#settingstab"+(0<rcmail.env.action.indexOf("identity")?"identities":rcmail.env.action.replace(/\./g,"")));$(a).addClass("selected").children().first().removeAttr("onclick").click(function(){return!1})}),"folders"==rcmail.env.action)(new rcube_splitter({id:"folderviewsplitter",p1:"#folderslist",p2:"#folder-details",orientation:"v",relative:!0,start:266,min:180,size:12})).init(),rcmail.addEventListener("setquota",D),J($("#folderslist"));else if("identities"==rcmail.env.action)(new rcube_splitter({id:"identviewsplitter",
...
else{if("edit-prefs"==rcmail.env.action){a=$("#preferences-details fieldset.advanced legend");var c=$('<a href="#toggle"></a>').text(rcmail.gettext("toggleadvancedoptions")).attr("title",rcmail.gettext("toggleadvancedoptions")).addClass("advanced-toggle");a.click(function(a){c.html($(this).hasClass("collapsed")?"&#9650;":"&#9660;");$(this).toggleClass("collapsed").closest("fieldset").children(".propform").toggle()}).append(c).addClass("collapsed");bw.mz&&a.parents("form").css("display","inline")}}else"addressbook"==
rcmail.env.task&&(rcmail.addEventListener("beforepushgroup",R).addEventListener("beforepopgroup",S).addEventListener("menu-open",B).addEventListener("menu-close",B),""==rcmail.env.action&&((new rcube_splitter({id:"addressviewsplitterd",p1:"#addressview-left",p2:"#addressview-right",orientation:"v",relative:!0,start:206,min:150,size:12,render:n})).init(),(new rcube_splitter({id:"addressviewsplitter",p1:"#addresslist",p2:"#contacts-box",orientation:"v",relative:!0,start:266,min:260,size:12})).init()),
a=$("#dragcontactmenu"),a.length&&(rcmail.gui_object("dragmenu","dragcontactmenu"),z.dragmenu=a));$(".tabbed").each(function(a,b){L(b)});$("select.decorated").each(function(){if(bw.opera)$(this).removeClass("decorated");else{var a=$(this),b=a.parent(),c=Math.max(a.height(),26)-2,f=a.width()-22,e=$("option",this).first().text();""!=$("option:selected",this).val()&&(e=$("option:selected",this).text());var n=$('<a class="menuselector" tabindex="-1"><span class="handle">'+e+"</span></a>").css("position",
"absolute").offset(a.position()).insertAfter(a);n.children().width(f).height(c).css("line-height",c-1+"px");"absolute"!=b.css("position")&&b.css("position","relative");a.width(n.width()).on(bw.mz?"change keyup":"change",function(){var a=$("option:selected",this).text();$(this).next().children().text(a)});a.on("focus",function(a){n.addClass("focus")}).on("blur",function(a){n.removeClass("focus")})}});a=$("body.minwidth");a.length&&a.css("min-width",$(".toolbar").width()+$("#quicksearchbar").width()+
$("#searchfilter").width()+30);window.onresize=r;r()};this.init_tabs=L;this.show_about=function(a){var b=$("<iframe>").attr({id:"aboutframe",src:rcmail.url("settings/about"),frameborder:"0"});h=Math.floor(.75*$(window).height());buttons={};supportln=$("#supportlink");supportln.length&&(v.supporturl=supportln.attr("href"))&&(buttons[supportln.html()]=function(a){0>v.supporturl.indexOf("mailto:")?window.open(v.supporturl):location.href=v.supporturl});b.dialog({modal:!0,resizable:!1,closeOnEscape:!0,
title:a?a.title||a.innerHTML:null,close:function(){b.dialog("destroy").remove()},buttons:buttons,width:640,height:h}).width(640)};this.show_popup=function(a,b,d){!d&&u[a]||E(a,d);d=u[a]||{};d=$(d.link?d.link:"#"+a+"link");var c=d.offset();d.has(".inner")&&(d=d.children(".inner"));return rcmail.command("menu-open",{menu:a,show:b},d.get(0),$.Event("click",{target:d.get(0),pageX:c.left,pageY:c.top,clientX:c.left,clientY:c.top}))};this.toggle_popup=function(a,b,d){!d&&u[a]||E(a,d);return rcmail.command("menu-open",
a,b.target,b)};this.add_popup=E;this.import_dialog=function(){var a=$("#uploadform").clone().removeClass("popupdialog");rcmail.simple_dialog(a,rcmail.gettext("importmessages"),function(b){return rcmail.command("import-messages",$(a.find("form")[0]))},{button:"import",closeOnEscape:!0,minWidth:400})};this.set_searchmod=function(a){var b=rcmail.env.task;var d=rcmail.env.search_mods,f=rcmail.env.mailbox;"all"==$('input[name="s_scope"]:checked').val()&&(f="*");d||(d={});if("mail"==b){d[f]||(d[f]=rcube_clone_object(d["*"]));
var e=d[f];b="text"}else e=d,b="*";a.checked?e[a.value]=1:delete e[a.value];a.value==b&&$('input:checkbox[name="s_mods[]"]').map(function(){this!=a&&(this.checked=!0,a.checked?(this.disabled=!0,delete e[this.value]):(this.disabled=!1,e[this.value]=1))});rcmail.set_searchmods(e)};this.set_searchscope=function(a){rcmail.set_searchscope(a.value)};this.show_header_row=C;this.hide_header_row=function(a){var b=$("#_"+a);G[a]=b.val();b.val("");$("#compose-"+a).hide();$("#"+a+"-link").show();t();return!1};
this.update_quota=D;this.get_pref=e;this.save_pref=q;this.folder_search_init=J;if(window.$&&document.body){var M=e("minimalmode");(parseInt(M)||null===M&&850>$(window).height())&&$(document.body).addClass("minimal");bw.tablet&&$("#viewport").attr("content","width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0");$(document).ready(function(){l.init()})}}
